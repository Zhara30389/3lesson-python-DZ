...
A_ent = input("Введите через пробел элементы списка: ").split()
A_num = list(map(int, A_ent))
if len(A_num) != N:
    print("Введенные элементы не соответсвуют заданному количеству ")
else:
...
if len(A_num) != N:
   ...
   X = int(input("Введите число X, с которым необходимо сравнить элементы списка: "))
    min = abs(X - A_num[0])
    index = 0
    for i in range(1, N):
        count = abs(X - A_num[i])
        if count < min:
            min = count
            index = i
print(f"Число {A_num[index]} в списке А наиболее близко по величине к числу {X}")
